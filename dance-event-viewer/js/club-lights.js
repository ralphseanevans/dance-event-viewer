...
(function () {
  ...
  function schedulePeeker(initial) {
    ...
    if (!peeker || !peekerEnabled() || motionQuery.matches || document.hidden) return;
    peekerTimer = window.setTimeout(showPeeker, initial ? random(2800, 5200) : random(9000, 17000));
  }

  function showPeeker() {
    if (!peeker || !peekerEnabled() || motionQuery.matches || document.hidden) return schedulePeeker(false);
    var x = random(12, 88);
  ...
  window.addEventListener("dev-page-character-change", scheduleAll);
  ...
  schedule();
  if (window.DEV_PAGE_CHARACTER_READY) window.DEV_PAGE_CHARACTER_READY.then(function () { schedulePeeker(true); });
  else schedulePeeker(true);
})();
