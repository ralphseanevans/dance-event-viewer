...
There is no second editable viewer under the private Dance Website folder. Public data,
site-log, share pages, and logo assets are generated by Daily Operating System's
`Tools\publish_site.ps1`: dry run first, then `-Publish`. The script reads the token
